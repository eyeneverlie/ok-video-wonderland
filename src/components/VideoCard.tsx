    ...
    >
      ...
      <div className="mt-3 px-1">
        <div className="flex gap-3">
          {/* Author Avatar */}
          <Link to={`/channel/${author.toLowerCase().replace(/\s+/g, '-')}`} className="h-9 w-9 flex-shrink-0">
            <img 
              src={authorAvatar} 
              alt={author}
          ...
          <div className="flex-1 min-w-0">
            <Link to={`/video/${id}`}>
              <h3 className="font-medium text-base leading-tight mb-1 line-clamp-2 text-balance hover:text-primary transition-colors">
                {title}
              </h3>
            </Link>
            <Link to={`/channel/${author.toLowerCase().replace(/\s+/g, '-')}`} className="block">
              <p className="text-sm text-muted-foreground hover:text-foreground transition-colors">{author}</p>
            </Link>
